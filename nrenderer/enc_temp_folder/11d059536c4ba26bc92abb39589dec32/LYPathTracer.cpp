...
namespace LYPathTracer
{
    ...
    void LYPathTracerRenderer::rayTask(RGBA* pixels, int width, int height, int off, int step) {
        for(int i=off; i<height; i+=step) {
            for (int j=0; j<width; j++) {
                ...
                rayTracing(ray, 0, energy, i, j, false);
    ...
    void LYPathTracerRenderer::photonTask(RGBA* pixels, int width, int height, int off, int step) {
        for (int i = off; i < photonNum; i += step) {
            ...
            photonTracing(ray, color, 0, 0, false);
        }
    }
    ...
    void LYPathTracerRenderer::rayTracing(const Ray& r, int currDepth, float lambda, int x, int y, bool in) {
        // reflect: continue rayTracing
        // refract: continue rayTracing
        // diffuse: stop rayTracing, store photon
        ...
        if (hitObject && hitObject->t < hitLight->t) {
            ...
            if (dynamic_pointer_cast<Insulator>(shaderPrograms[mtlHandle.index()])) {
                ...
                //    in ? -hitObject->normal : hitObject->normal,
                //    scattered.attenuation,  // color
                //    lambda * glm::dot(in ? -hitObject->normal : hitObject->normal, scattered.ray.direction) / scattered.pdf * F,    // strength
                //    x, y));
                //mtx.unlock();
                ...
                else
                {
                    ...
                    //viewPoints.push_back(ViewPoint(hitObject->hitPoint,
                    //    in ? -hitObject->normal : hitObject->normal,
                    //    refract.attenuation,  // color
                    //    lambda * glm::dot(in ? hitObject->normal : -hitObject->normal, refract.ray.direction) / refract.pdf * F,    // strength
                    //    x, y));
                    //mtx.unlock();
                    //float fra_n_dot_in = glm::dot(in ? hitObject->normal : -hitObject->normal, refract.ray.direction);
                    float fra_pdf = refract.pdf;
                    rayTracing(refract.ray, currDepth+1, lambda*fra_pdf, x, y, in ? false : true);
    ...
    void LYPathTracerRenderer::photonTracing(const Ray& r, Vec3 rColor, int currDepth,int diffCount, bool in) {
        if (currDepth >= depth) {
            return;
        }
        auto hitObject = closestHitObject(r);
        ...
        if (hitObject && hitObject->t < hitLight->t) {
            ...
            if (dynamic_pointer_cast<Insulator>(shaderPrograms[mtlHandle.index()])) {
                ...
                photonTracing(scattered.ray, scattered.attenuation*rColor * F, currDepth + 1, diffCount + 1, in);
            ...
            photonTracing(scattered.ray, scattered.attenuation * rColor, currDepth + 1, diffCount + 1, in);
